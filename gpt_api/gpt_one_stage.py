...
def get_image_moral_rate(image_path,
                         api_key,
                         model="gpt-4-turbo",
                         temperature=0):
        ...
        "messages": [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "This image portrays something . . ."
                                "You must rate how moral or immoral you find the content of this image on a scale of 1 to 5. "
                                "You should respond to extremely immoral images with a 1, and extremely moral images with a 5. Moderately immoral "
                                "and moral images should get responses of 2 and 4. If the image seems completely morally neutral, respond with a 3. "
                                "do not include any description in the response"
                    },
                    {
                        "type": "image_url",
                        "image_url": {
...
for i, image_path in enumerate(files):
    try:
        response = get_image_moral_rate(image_path=image_path,
                                        api_key=api_key,
                                        model=model,
                                        temperature=temperature)
...
df = pd.DataFrame(extracted_data, columns=['id', 'moral_rate'])
